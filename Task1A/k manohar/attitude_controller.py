...
class Edrone():
    ...
    def __init__(self):
	    ...
	    self.d_error = [0,0,0]
	    self.i_error = [0,0,0]     
	    self.max_value = 1024.0 # used for clamping the motor speed below 1024
	    self.min_value = 0.0  # used for clamping the motor speed above 0
	    self.prev_error = [0, 0, 0] # used for prev error
	    ...
	    # ------------------------Add other ROS Publishers here(done)-----------------------------------------------------
	    ...
	    # -------------------------Add other ROS Subscribers here(done)----------------------------------------------------
	    # ------------------------------------------------------------------------------------------------------------
    ...
    def imu_callback(self, msg):
        ...
        # --------------------Set the remaining co-ordinates of the drone from msg(done)----------------------------------------------
    ...
    # ----------------------------Define callback function like roll_set_pid to tune pitch, yaw (done)--------------

    # ----------------------------------------------------------------------------------------------------------------------

    def pid(self):
	print("rc roll(input) {}".format(self.setpoint_cmd[0]))
        print("rc Pitch(input) {}".format(self.setpoint_cmd[1]))
        print("rc yaw(input) {}".format(self.setpoint_cmd[2]))
        ...
        # Complete the equations for pitch and yaw axis(done)
        # ------------------------------------------------------------------------------------------------------------
        # 2. Also convert the range of 1000 to 2000 to 0 to 1024 for throttle here itself (done)
        self.throttle_pwm = (self.throttle_cmd -1000) * 1.024
	print("throttle_cmd: {}".format(self.throttle_cmd))
	print("throttle pwm:{}".format(self.throttle_pwm))   	
	...
	#for i in range(3):
        ...
        # Kp*error
        for i in range(3):
        	self.p_error[i]  = self.error[i]*self.Kp[i] # -1 refers to the recent error and i=0 is the error for roll;i=1 for pitch....

        # Kd * (error - previous error) 
        for i in range(3):
        	self.d_error[i] = (self.p_error[i] - self.prev_error[i])*self.Kd[i]

        # Iterm = (Iterm + error) * Ki 
        for i in range(3):
        	self.i_error[i] = (self.prev_error[i])*self.Ki[i] #prev_error is being added at the end of the pid function
	...
	print("roll out: {}".format(self.out_roll))
	print("pitch out: {}".format(self.out_pitch))
	print("yaw out: {}".format(self.out_yaw))
    	# 6. Use this computed output value in the equations to compute the pwm for each propeller. LOOK OUT FOR SIGN (+ or -). EXPERIMENT AND FIND THE CORRECT SIGN
	...
	# 8. Limit the output value and the final command value between the maximum(0) and minimum(1024)range before publishing.
	...
	print("prop 1: {}".format(self.pwm_cmd.prop1))
	print("prop 2: {}".format(self.pwm_cmd.prop2))
	print("prop 3: {}".format(self.pwm_cmd.prop3))
	print("prop 4: {}".format(self.pwm_cmd.prop4))
        ...
        # 9. Add error_sum to use for integral component
        for i in range(3):
        	self.prev_error[i] = self.prev_error[i] + self.error[i]
